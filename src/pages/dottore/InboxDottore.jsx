...
export default function InboxDottore() {
    ...
    const loadMessages = () => {
       ...
       if (stored) {
         ...
         if (targetId) {
             ...
             if (found) {
                 // FIX: Passo i messaggi aggiornati invece di usare lo state
                 handleSelectMessageFromLoad(found, parsedMsgs); 
             }
  ...
  // --- AZIONI ---
  const markAsRead = (msgId, currentMessages = null) => {
    // Usa i messaggi passati o lo stato corrente
    const msgsToUse = currentMessages || messages;
    const msg = msgsToUse.find(m => m.id === msgId);
    if (msg && msg.read) return;

    const updated = msgsToUse.map(m => m.id === msgId ? { ...m, read: true, unread: false } : m);
    setMessages(updated);
    localStorage.setItem('mimesi_doctor_inbox', JSON.stringify(updated));
  };

  // Funzione per gestire selezione durante il load iniziale
  const handleSelectMessageFromLoad = (msg, currentMessages) => {
    setSelectedMsg(msg);
    setShowOtp(false);
    setOtpCode('');
    if (!msg.read) {
      markAsRead(msg.id, currentMessages); // Passa i messaggi aggiornati
    }
  };
  ...
  const handleSelectMessage = (msg) => {
    ...
    setOtpCode('');
    if (!msg.read) {
      markAsRead(msg.id); // Usa lo stato corrente (messages già aggiornato dal polling)
    }
  };
  ...
  // --- RIFIUTO PREVENTIVO ---
  const handleReject = () => {
    if (window.confirm('Sei sicuro di voler rifiutare questo preventivo?\n\nLa lavorazione verrÃ  archiviata come "Rifiutata" e sia tu che il laboratorio riceverete notifica.')) {
